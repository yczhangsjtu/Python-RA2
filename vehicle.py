...
class Vehicle(MobileUnit):
  ...
  def drawBloodBar(self, screen):
    if self.HP >= self.fullHP/2:
      ...
      screen.blit(vehicleHealthBlood.subsurface(
          0, 0, ngrid*3+1, 5), (self.x-36, self.y-50))
    elif self.HP >= self.fullHP/4:
      ngrid = self.HP * 24 / self.fullHP
      screen.blit(vehicleHurtBlood.subsurface(
          0, 0, ngrid*3+1, 5), (self.x-36, self.y-50))
    else:
      ngrid = self.HP * 24 / self.fullHP
      screen.blit(vehicleDangerBlood.subsurface(
          0, 0, ngrid*3+1, 5), (self.x-36, self.y-50))
