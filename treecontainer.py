...
class TreeContainer(object):
  ...
  def toString(self, indent):
    res = "%s|----[%s,%s,%s,%s]" % (" "*indent, self.x,
                                    self.y, self.x+self.width,
                                    self.y+self.height)
    if self.data is not None:
      res += "--------(%d,%d),%d" % (self.data.offsetx,
                                     self.data.offsety, self.data.size)
    if not self.leaf:
  ...
  def collide(self, size, x, y):
    lf = occupyx * size
    w = occupyy * size
    c1 = (x > self.x-lf and x < self.x+self.width+lf and
          y > self.y-w and y < self.y+self.height+w)
    X = 2*x - (2*self.x + self.width)
    Y = 2*y - (2*self.y + self.height)
    c2 = abs(w*X)+abs(lf*Y) < abs(lf*self.height+w*self.width+2*lf*w)
    return c1 and c2
  ...
  def available(self, data, x, y):
    if self.leaf:
      if self.data == data or self.data is None:
        return True
